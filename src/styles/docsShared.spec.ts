...
test('docs-shared: sidebar tracks header pad; wide screens bump both', (assert) => {
    ...
    assert.equal(styles.includes('--sidebar-link-pad-inline: 0.7rem'), true);
...
test('demo sidebar hover: text-only like Starlight (no tint fill)', (assert) => {
    ...
    assert.equal(hover.includes('color: var(--text)'), true);
...
test('demo sidebar nav: match docs starlight-custom metrics', (assert) => {
    ...
    assert.equal(docs.includes('margin-top: 0.85rem'), true);
    assert.equal(docs.includes('padding-inline: 0.65rem'), true);
    assert.equal(docs.includes('border-radius: 0.45rem'), true);

    assert.equal(/\.sidebar-top-level\s*>\s*li\s*\+\s*li\s*\{[^}]*margin-top:\s*0\.85rem/.test(styles), true);
    assert.equal(styles.includes('--sidebar-link-pad-inline: 0.7rem'), true);
    ...
    assert.equal(/\.sidebar-group summary\s*\{[^}]*padding:\s*0\.35em\s*var\(--sidebar-link-pad-inline\)/.test(styles), true);
...
test('explorer sidebar search: full-bleed band above Home', (assert) => {
    ...
    assert.equal(/\.sidebar-nav\s*\{[^}]*padding:\s*0\.65rem var\(--sidebar-pad-x\)/.test(styles), true);
...
test('header contract buttons: tight icon gap + Callspec mark color (not host accent)', (assert) => {

    const styles = readFileSync(path.join(root, 'src/callspec-ui/ui/styles.css'), 'utf8');
    const shared = readFileSync(path.join(root, 'src/styles/docs-shared.css'), 'utf8');

    // Icon↔label gap must stay tight (0.45rem looked too wide).
    assert.equal(/\.header-contract-btn\s*\{[^}]*gap:\s*0\.2rem/.test(styles), true);
    assert.equal(/\.header-contract-btn\s*\{[^}]*gap:\s*0\.45rem/.test(styles), false);

    // Callspec mark follows brand assets (mark-light / mark-dark), not API --accent
    // (Chirp teal would otherwise tint the hex mark).
    assert.equal(shared.includes('--docs-callspec-mark: #0284c7'), true);
    assert.equal(shared.includes('--docs-callspec-mark: #22d3ee'), true);
    assert.equal(
