...
Chirp is Callspec's sample API &mdash; a fictional social app (**Chirp API v2**). Use it to click around the explorer, fire live RPCs, and connect MCP.
...
That process is Callspec's built-in explorer on a real Chirp server &mdash; try-it, `callspec.json`, OpenAPI, and `/mcp`.
