...
Callspec doesn't care about your folders. A single file is fine for demos — see [Minimal single-file server example](./single-file-server-example.md). When the API grows (or you're past a toy), **split** so each route, shared pred, and the registry stay easy to find and test.
