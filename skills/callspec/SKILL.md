...
7. Don’t wire Express error middleware or `express.json()` on the `mountSpec` router — it owns JSON parse and the catch path. Host middleware outside the mount: [Outside Callspec](../../src/content/docs/outside-callspec.md). Use mountSpec `{ json: false }` or `{ json: { limit } }` only to opt out or configure.
