...
Not “generate a client.” One TypeScript registry (`defineRoute` + runtyp) that *is* the Express RPC server, boundary validation, white-label `/docs`, native `callspec.json`, OpenAPI projection, same-handler MCP at `/mcp`, and TS client/validator codegen so the browser never imports the server.
...
| Form / shared validation | Not the product | `exports` → runtyp preds for React |
